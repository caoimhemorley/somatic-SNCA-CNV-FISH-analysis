...
plot_dir <- plots_reg_sox10neg_loss
stats_dir <- stats_reg_sox10neg_loss
...
cnv_clean <- cnv_data %>%
  ...
  dplyr::filter(!is.na(Percentage_SOX10neg_loss))
...
region_tests_across_groups <- function(df_region) {
  kw <- stats::kruskal.test(Percentage_SOX10neg_loss ~ Classification, data = df_region)
  
  dunn <- dunn.test::dunn.test(
    x = df_region$Percentage_SOX10neg_loss,
...
plot_region_across_groups <- function(df_region, brackets, region_label) {
  ggplot2::ggplot(
    df_region,
    ggplot2::aes(x = Classification, y = Percentage_SOX10neg_loss, fill = Classification)
  ) +
    ggplot2::annotate(
      "rect",
...
for (reg in regions_present) {
  ...
  ggplot2::ggsave(
    file.path(plot_dir, paste0("SOX10neg_Losses_", reg, "_AcrossGroups.png")),
...
ggplot2::ggsave(
  file.path(plot_dir, "SOX10neg_Losses_AcrossGroups_Combined.png"),
...
readr::write_tsv(
  ...
  file.path(stats_dir, "SOX10neg_Losses_AcrossGroupsWithinRegion_Kruskal.tsv")
)

readr::write_tsv(
  dplyr::bind_rows(all_dunn_region),
  file.path(stats_dir, "SOX10neg_Losses_AcrossGroupsWithinRegion_Dunn.tsv")
)
...
within_group_across_regions_opca <- function(df) {
  ...
  fit <- stats::aov(Percentage_SOX10neg_loss ~ Region, data = df_opca)
...
within_group_across_regions_kw_dunn <- function(df, cls) {
  ...
  kw <- stats::kruskal.test(Percentage_SOX10neg_loss ~ Region, data = df_cls)
  
  dunn <- dunn.test::dunn.test(
    x = df_cls$Percentage_SOX10neg_loss,
    g = df_cls$Region,
    method = "bonferroni",
    kw = FALSE,
    list = TRUE
...
readr::write_tsv(opca_res$anova, file.path(stats_dir, "SOX10neg_Losses_OPCA_Regions_ANOVA.tsv"))
readr::write_tsv(opca_res$tukey, file.path(stats_dir, "SOX10neg_Losses_OPCA_Regions_Tukey.tsv"))

readr::write_tsv(snd_res$kw, file.path(stats_dir, "SOX10neg_Losses_SND_Regions_Kruskal.tsv"))
readr::write_tsv(snd_res$dunn, file.path(stats_dir, "SOX10neg_Losses_SND_Regions_Dunn.tsv"))

readr::write_tsv(ctl_res$kw, file.path(stats_dir, "SOX10neg_Losses_Control_Regions_Kruskal.tsv"))
readr::write_tsv(ctl_res$dunn, file.path(stats_dir, "SOX10neg_Losses_Control_Regions_Dunn.tsv"))
...
summary_tbl <- cnv_clean %>%
  ...
  dplyr::summarise(
    ...
    mean = mean(Percentage_SOX10neg_loss, na.rm = TRUE),
    sd = stats::sd(Percentage_SOX10neg_loss, na.rm = TRUE),
    median = stats::median(Percentage_SOX10neg_loss, na.rm = TRUE),
    iqr_low = stats::quantile(Percentage_SOX10neg_loss, 0.25, na.rm = TRUE),
    iqr_high = stats::quantile(Percentage_SOX10neg_loss, 0.75, na.rm = TRUE),
    min = min(Percentage_SOX10neg_loss, na.rm = TRUE),
    max = max(Percentage_SOX10neg_loss, na.rm = TRUE),
    .groups = "drop"
  )

readr::write_tsv(
  summary_tbl,
  file.path(stats_dir, "SOX10neg_Losses_ByClassification_ByRegion_Summary.tsv")
)
