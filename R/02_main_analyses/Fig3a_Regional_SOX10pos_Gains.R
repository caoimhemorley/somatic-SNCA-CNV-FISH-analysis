...
plot_dir <- plots_reg_sox10pos_gain
stats_dir <- stats_reg_sox10pos_gain
...
cnv_clean <- cnv_data %>%
  ...
  dplyr::filter(!is.na(Percentage_SOX10pos_gain))
...
region_tests_across_groups <- function(df_region) {
  kw <- stats::kruskal.test(Percentage_SOX10pos_gain ~ Classification, data = df_region)
  
  dunn <- dunn.test::dunn.test(
    x = df_region$Percentage_SOX10pos_gain,
...
plot_region_across_groups <- function(df_region, brackets, region_label) {
  ...
  ggplot2::ggplot(
    df_region,
    ggplot2::aes(x = Classification, y = Percentage_SOX10pos_gain, fill = Classification)
  ) +
    ggplot2::annotate(
      "rect",
...
for (reg in regions_present) {
  ...
  ggplot2::ggsave(
    filename = file.path(plot_dir, paste0("SOX10pos_Gains_", reg, "_AcrossGroups.png")),
...
ggplot2::ggsave(
  filename = file.path(plot_dir, "SOX10pos_Gains_AcrossGroups_Combined.png"),
  plot = combined_plot,
  width = 14,
  height = 7,
  dpi = 600
...
readr::write_tsv(
  kw_region_tbl,
  file.path(stats_dir, "SOX10pos_Gains_AcrossGroupsWithinRegion_Kruskal.tsv")
)

readr::write_tsv(
  dunn_region_tbl,
  file.path(stats_dir, "SOX10pos_Gains_AcrossGroupsWithinRegion_Dunn.tsv")
)
...
within_group_across_regions_opca <- function(df) {
  ...
  fit <- stats::aov(Percentage_SOX10pos_gain ~ Region, data = df_opca)
...
within_group_across_regions_kw_dunn <- function(df, cls) {
  ...
  kw <- stats::kruskal.test(Percentage_SOX10pos_gain ~ Region, data = df_cls)
  
  dunn <- dunn.test::dunn.test(
    x = df_cls$Percentage_SOX10pos_gain,
    g = df_cls$Region,
    method = "bonferroni",
    kw = FALSE,
    list = TRUE
...
readr::write_tsv(
  opca_res$anova,
  file.path(stats_dir, "SOX10pos_Gains_OPCA_Regions_ANOVA.tsv")
)
readr::write_tsv(
  opca_res$tukey,
  file.path(stats_dir, "SOX10pos_Gains_OPCA_Regions_Tukey.tsv")
)

readr::write_tsv(
  snd_res$kw,
  file.path(stats_dir, "SOX10pos_Gains_SND_Regions_Kruskal.tsv")
)
readr::write_tsv(
  snd_res$dunn,
  file.path(stats_dir, "SOX10pos_Gains_SND_Regions_Dunn.tsv")
)

readr::write_tsv(
  ctl_res$kw,
  file.path(stats_dir, "SOX10pos_Gains_Control_Regions_Kruskal.tsv")
)
readr::write_tsv(
  ctl_res$dunn,
  file.path(stats_dir, "SOX10pos_Gains_Control_Regions_Dunn.tsv")
)
...
summary_tbl <- cnv_clean %>%
  ...
  dplyr::summarise(
    ...
    mean = mean(Percentage_SOX10pos_gain, na.rm = TRUE),
    sd = stats::sd(Percentage_SOX10pos_gain, na.rm = TRUE),
    median = stats::median(Percentage_SOX10pos_gain, na.rm = TRUE),
    iqr_low = stats::quantile(Percentage_SOX10pos_gain, 0.25, na.rm = TRUE),
    iqr_high = stats::quantile(Percentage_SOX10pos_gain, 0.75, na.rm = TRUE),
    min = min(Percentage_SOX10pos_gain, na.rm = TRUE),
    max = max(Percentage_SOX10pos_gain, na.rm = TRUE),
    .groups = "drop"
  )

readr::write_tsv(
  summary_tbl,
  file.path(stats_dir, "SOX10pos_Gains_ByClassification_ByRegion_Summary.tsv")
)
