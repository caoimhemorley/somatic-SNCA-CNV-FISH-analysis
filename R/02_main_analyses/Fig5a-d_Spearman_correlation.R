...
make_correlation_plot <- function(df, x_var, y_var, group_var, x_label, y_label) {
  ...
  color_mapping <- setNames(
    c("aquamarine4", "darkorchid4", "gray70")[seq_along(groups)],
    groups
  )
  
  p <- ggplot2::ggplot(df, ggplot2::aes(x = .data[[x_var]], y = .data[[y_var]], color = .data[[group_var]])) +
...
age_onset_pos_df <- avg_cnv_data_msa_only %>%
  ...
  dplyr::mutate(
    group = dplyr::if_else(Classification == "OPCA", "OPCA Cerebellum", "SND Putamen")
  )
...
plot1 <- make_correlation_plot(
  ...
  x_label = "Most affected region (% SOX10+ SNCA gains)",
  y_label = "Least affected region (% SOX10+ SNCA gains)"
)
...
plot2 <- make_correlation_plot(
  ...
  y_label = "Average % SOX10+ SNCA gains"
)
...
plot3 <- make_correlation_plot(
  ...
  group_var = "group",
  x_label = "% SOX10- SNCA gains",
  y_label = "% SOX10+ SNCA gains"
)
...
ggplot2::ggsave(
  filename = file.path(cor_plot_dir, "FISH_Spearman_Correlation_Most_vs_Least_Affected_SOX10pos_SNCA_gain.png"),
  plot = plot1,
  width = 6,
  height = 6,
  dpi = 600
...
ggplot2::ggsave(
  filename = file.path(cor_plot_dir, "FISH_Spearman_Correlation_Avg_SOX10pos_SNCA_gain_vs_Age_of_onset.png"),
  plot = plot2,
  width = 6,
  height = 6,
  dpi = 600
...
ggplot2::ggsave(
  filename = file.path(cor_plot_dir, "FISH_Spearman_Correlation_SOX10neg_SNCA_gain_vs_SOX10pos_SNCA_gain.png"),
  plot = plot3,
  width = 6,
  height = 6,
  dpi = 600
...
ggplot2::ggsave(
  filename = file.path(cor_plot_dir, "FISH_Spearman_Correlation_Avg_SOX10neg_SNCA_gain_vs_Age_of_onset.png"),
  plot = plot4,
  width = 6,
  height = 6,
  dpi = 600
...
ggplot2::ggsave(
  filename = file.path(cor_plot_dir, "FISH_Spearman_Correlation_SOX10_SNCA_gain_Combined_2x2.png"),
  plot = combined_plot,
  width = 12,
  height = 12,
  dpi = 600
